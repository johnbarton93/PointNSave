$(document).ready(function(){
  ...
  $('.choosebtn').on('click', function(e) {
    var tid = $(this).parent('.btn-block2').parent('.tcont').find('.template-block').attr('id');
    var ttext = $(this).parent('.btn-block2').parent('.tcont').find('.template-block').find('.title').text();
    cookies.set('templateId', tid);
    cookies.set('templateName', ttext);
  });
...
/*
  ...
  if(templateId=='template3') {
    ...
    $(".tabinp").each(function(index) {
      if($(this).attr('id')!='tab30') {
        $('template-block .tabinp').attr('maxlength','10');
      }
    });
  }
...
$('.insert').on('click', function(e) {
  var lineid = $(this).parent('.funcs').parent('.line').find('.tab').attr('id');

  if(!$(this).hasClass('sel')) {
    $(this).addClass('sel');
    $('.' + lineid + '-square').addClass('show');
    cookies.set(lineid + 'del', '1');

  } else if($(this).hasClass('sel'))  {
    $(this).removeClass('sel');
    $('.' + lineid + '-square').removeClass('show');
    cookies.set(lineid + 'del', '0');
  }
});

  var opt1del = cookies.get('opt1del');
  var opt2del = cookies.get('opt2del');
  var opt3del = cookies.get('opt3del');
  var opt4del = cookies.get('opt4del');
  ...
  if(opt1del==1) {
    $('.opt1-square').addClass('show');
    $('.opt1').addClass('sel');
  } else if(opt1del==0) {
    $('.opt1-square').removeClass('show');
    $('.opt1').removeClass('sel');
  }

  if(opt2del==1) {
    $('.opt2-square').addClass('show');
    $('.opt2').addClass('sel');
  } else if(opt2del==0) {
    $('.opt2-square').removeClass('show');
    $('.opt2').removeClass('sel');
  }

  if(opt3del==1) {
    $('.opt3-square').addClass('show');
    $('.opt3').addClass('sel');
  } else if(opt3del==0) {
    $('.opt3-square').removeClass('show');
    $('.opt3').removeClass('sel');
  }

  if(opt4del==1) {
    $('.opt4-square').addClass('show');
    $('.opt4').addClass('sel');
  } else if(opt4del==0) {
    $('.opt4-square').removeClass('show');
    $('.opt4').removeClass('sel');
  }
